...
all: $(BUILD_DIR)/NSENGINE.OBX $(BUILD_DIR)/autorun.sys

$(BUILD_DIR):
	mkdir -p $(BUILD_DIR)

$(BUILD_DIR)/NSENGINE.OBX: handler/netstream.s | $(BUILD_DIR)
	$(MADS) handler/netstream.s -i:handler/include -d:BASEADDR=$(HANDLER_BASE) -d:HIBUILD=0 -s -p -o:$@

$(BUILD_DIR)/autorun.sys: tests/netstream_smoke.c tests/netstream_api.s tests/atari_netstream.cfg | $(BUILD_DIR)
